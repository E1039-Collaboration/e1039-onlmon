...
DIR_IN=/data4/e1039_data/online/intime_hodo/run_$(printf '%06d' $RUN)
...
FILE_IN=$DIR_IN/hodo_in_time.txt
if [ ! -e $FILE_IN  ] ; then
...
FILE_DPHODO_OUT=$DIR_OUT/dphodo.tsv
FILE_TRIG_OUT=$DIR_OUT/trig.tsv
rm -rf $DIR_OUT
#rm $FILE_HODO_OUT $FILE_TRIG_OUT
mkdir -p $DIR_OUT
while read DET_NAME DET_ID ELE_ID TIME STATUS ; do
    ...
    elif [ ${DET_NAME:0:2} = 'DP' ] ; then
	echo -e "$LINE" >>$FILE_DPHODO_OUT
    else
	echo -e "$LINE" >>$FILE_TRIG_OUT
    fi
